	...
	api.actionSheet({
		...
		switch(index){
			case 1:
                ...
                targetWidth:500,
                targetHeight:500,
                saveToPhotoAlbum: true}, function(ret, err){ 
                if (ret) {
                    pic1path = ret.data;
                    // var img = document.getElementById('pic1');
                    // img.src = pic1path;
                            // $api.setStorage('pic1path',pic1path);
                    uploadpic();     
                    }
                });
				break;
...
var now = Date.now();
        var appKey = sha1("A6990997740260"+"UZ"+"91F2D4DC-9D5D-448C-1F11-DD1B2BDAF063"+"UZ"+now)+"."+now
                    api.ajax({
                    ...
                    returnAll:false,
                     headers:{
                    "X-APICloud-AppId": 'A6990997740260',
    	...
    	// 					"filename": ret.filename
					...
					// });
						var retid = ret.id;
                    	var returl = ret.url;
                            $api.setStorage('touxiangurl',returl);
                    	var retxuehao = $api.getStorage('xuehao');
                    	var retname = $api.getStorage('name');
                    	var model = api.require('model');
						model.config({
							// appId：'A6990997740260',
						    appKey: '91F2D4DC-9D5D-448C-1F11-DD1B2BDAF063',
